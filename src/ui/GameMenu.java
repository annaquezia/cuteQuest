...
public class GameMenu {
    ...
    private final KennelDAO kennelDAO = new KennelDAO();
    private final PetDAO petDAO = new PetDAO();

    public void start() {
        System.out.println("=========== LITTLE CATS PET SHOP ===========\n");
        Pet current;
        if (kennelDAO.getAllKennel().isEmpty()) {
    ...
    public void listPets() {
        ...
        for (int i = 0; i < pets.size(); i++) {
            ...
            String kennelName = "-";
            if (pet.getKennelID() > 0) {
                var kennel = kennelDAO.getKennel(pet.getKennelID());
                if (kennel != null) {
    ...
    public Pet createPet() {
        ...
        PetCreator creator = PetFactoryProvider.forType(petType, petDAO, kennelDAO);
